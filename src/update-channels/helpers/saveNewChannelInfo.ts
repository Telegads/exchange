...
export const saveNewChannelInfo = async (channelId: string, newChannelInfo: ChannelInfoResponse) =>
  channelRepository.updateChannel({
    ...
    er:
      newChannelInfo.views_last_30_days && newChannelInfo.posts_last_30_days && newChannelInfo.subs_count
        ? Math.round(
            (newChannelInfo.views_last_30_days / newChannelInfo.posts_last_30_days / newChannelInfo.subs_count) * 100,
          )
        : 0,
  });
